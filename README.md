...
### `setUserData(externalId:)` on Android

`setUserData(externalId:)` is **functional on iOS** but is currently a **no-op on Android** (a warning is printed). The Facebook Android SDK does not expose public support for passing `external_id` through `AppEventsLogger.setUserData(...)` yet. Track the upstream request at [facebook-android-sdk#1239](https://github.com/facebook/facebook-android-sdk/issues/1239).
